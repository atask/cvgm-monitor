...
tap.test('monitor should a connection event once connected to cvgm.net', t => {
...
tap.test('monitor emits an error event if connection is unsuccessful', t => {
  let fetchStub = sinon.stub()
  fetchStub.withArgs('http://www.cvgm.net')
    .returns({ ok: false })
  let CVGMMonitor = proxyquire('../monitor.js', { 'node-fetch': fetchStub })
...
tap.test('monitor should emit when a new song plays', t => {
  let fetchStub = sinon.stub()
  fetchStub.withArgs('http://www.cvgm.net')
    .returns({ ok: true, text: () => fixtureIndex })
...
tap.test('monitor retries current poll id when connection unsuccessful', t => {
